...
+'<tr><td>new</td><td>new</td><td>-</td></tr></table><table border="1">'
+'<br><a><img src="https://i.postimg.cc/yNd9hQDd/ooc.jpg" width="250" height="120"></a>'
+'<br><tr style="color:#a00"><td colspan="3">Дальше перечислим неперегружаемые операторы</td></tr>'
+'<tr><td style="color:blue">Оператор</td><td style="color:braun">Название</td></tr>'
+'<tr><td>.</td><td>Выбор члена</td></tr>'
+'<tr><td>.*</td><td>Выбор члена по указателю</td></tr>'
+'<tr><td>::</td><td>Оператор расширения области видимости</td></tr>'
+'<tr><td>:?</td><td>Оператор условия</td></tr>'
+'<tr><td>#</td><td>Препроцессорный символ</td></tr>'
+'<tr><td>##</td><td>Препроцессорный символ</td></tr>'
+'</table>'
+'</li>'
+'<li><ol><i>операторные функции должны подчиняться следующим требованиям:</i>'
+'<li>функция должна быть нестатической, либо принимать аргумент типа класса</li>'
+'<li>операторная функция не меняет число аргументов и приоритеты операций</li>'
+'<li>операторная функция объявленная как функция-член, не может иметь параметров.'
+'Если объявлена как глобальная, то может иметь один параметр</li>'
+'<li>функция бинарного оператора в качестве функции-члена должна иметь один параметр. '
+'Если как глобальная, то два параметра.</li>'
+'<li>операторная функция не может иметь параметров по умолчанию</li>'
+'<li>первый параметр должен иметь тип класса объекта</li>'
+'<li>операторные функции класса на следуются его производным классом</li>'
+'<li>функции =,(),[] и -> должны быть нестатическими</li>'
+'<li>Операторы new и delete рассматриваются отдельно от общей массы</li>'
+'</ol></li></ul>'
+'<ol><h4>Перегрузка бинарных операторов</h4>'
+'<i>функция бинарного оператора объявляется в виде:</i><br>'
+'<возвр_тип> operatorX(<тип_пар> par);<i> par - параметр</i><br>'
+'<i>Если функция объявляется, как глобальная, то это уже: </i><br>'
+'<возвр_тип> operatorX(<тип_пар1> par1, <тип_пар2>) par2);<br>'
+'<li><i>перегрузим операторы вычитания и сложения для класса Coord:</i><br>'
+'#include<iostream><br>'
+'#include<cstdlib><br>'
+'#include<cstdio><br>'
+'using namespace std;<br>'
...
+'	Coord(int _x,int _y){x=_x;y=_y;}<br>'
	+'void GetCoord(int& _x,int& _y){_x=x;_y=y;}<br>'
+'	Coord operator+(Coord& ob);<br>'
	+'Coord operator-(Coord& ob);	};<br>'
+'Coord<br>'
+'Coord::operator+(Coord& ob){<br>'
+'	Coord tempOb;<br>'
...
+'main(){<br>'
	...
	+'    setlocale(LC_ALL,"Rus");<br>'
+'	    int x,y;<br>'
	    +'Coord PtA(10,20),PtB(3,8),PtC;<br>'
	  +'  <i>//вызываем перегруженный оператор сложения</i><br>'
	+'    PtC=PtA+PtB;<br>'
+'	    PtC.GetCoord(x,y);<br>'
	    +'cout<<"PtC.x= "<<x<br>'
	  +'      <<" PtC.y= "<<y<<endl;<br>'
	+' <i>    //вызываем перегруженный оператор вычитания</i><br>'
+'		 PtC=PtA-PtB;<br>'
		 +'PtC.GetCoord(x,y);<br>'
		+' cout<<"PtC.x= "<<x<<" PtC.y= "<<y<<endl;<br>'   
	+'	 return 0;<br>'
+'} <i>//PtC.x= 13 PtC.y= 28</i><br>'
+'<i>//PtC.x= 7 PtC.y= 12</i><br></li></ol>'
+'<ul><h3>Продолжим издевательное издевательство</h3><li>'
+'<i>посмотрим на параметр операторной функции перечисляемого типа данных</i><br>'
+'#include<iostream><br>'
+'using namespace std;<br>'
+'class Coord{<br>'
	+'int x,y;<br>'
...
+'	system("color 2f");<br>'
	...
	+'    int x,y;<br>'
+'	    Coord PtA(10,20),PtB(3,8),PtC;<br>'
	    +'<i>//вызов оператора сложения</i><br>'
	  +'  PtC=PtA+PtB;<br>'
	+'    PtC.GetCoord(x,y);<br>'
+'	    cout<<"PtC.x= "&lt;&lt;x<br>'
	    +'    &lt;&lt;" PtC.y= "&lt;&lt;y&lt;&lt;endl;<br>'
	  +'   <i>//вызывем перегруженный оператор сложения</i><br>'
	+'	 PtC=PtA+30;<br>'
+'		 PtC.GetCoord(x,y);<br>'
		+' cout &lt; &lt;"(PtA+100).x= "&lt;&lt;x&lt;&lt;" (PtA+100).y= "&lt;&lt;y&lt;&lt;endl;<br>'   
	+'	 return 0;<br>'
+'} <i>//PtC.x= 13 PtC.y= 28</i><br>'
+'  <i>//(PtA+100).x= 40 (PtA+100).y= 50</i><br></li><li>'
+'<a>Перегрузка операторов отношения и логических операторов</a><br>'
+'<i>Пример в котором перегружаются операторы == и &&:</i><br>'
+'#include<iostream><br>'
+'using namespace std;<br>'
+'class Coord{<br>'
	+'int x,y;<br>'
+'   public:<br>'
  ...
  +'  Coord(){x=0;y=0;}<br>'
+'	void GetCoord(int& _x,int& _y){_x=x;_y=y;}<br>'
	+'bool operator==(Coord ob);<br>'
	+'bool operator&&(Coord ob);};<br>'
+'bool<br>'
+'Coord::operator==(Coord ob){<br>'
	+'return(x==ob.x && y==ob.y);}<br>'
+'bool<br>'
+'Coord::operator&&(Coord ob){<br>'
	+'return(x && ob.x)&&(y && ob.y);}<br>'
+'int main(){<br>'
+'	system("color 2f");<br>'
	+'    setlocale(LC_ALL,"Rus");<br>'
+'	Coord Pt1(10,20),Pt2(10,25),<br>'
	  +'      Pt3(10,20),Pt4;<br>'
	+'if(Pt1==Pt2) cout<<"Pt1 raven Pt2\n";<br>'
+'	else cout<<"Pt1 ne raven Pt2\n";<br>'
	  +' if(Pt1==Pt3)cout<<"Pt1 raven Pt3\n";<br>'
	+'   else cout<<"Pt1 ne raven Pt3\n";<br>'
+'	if(Pt1 && Pt3)<br>'
	  +' cout<<"Pt1 && Pt3 istinno\n";<br>'
	+'else cout<<"Pt1 && Pt2 logno\n";<br>'
+'	   if(Pt1 && Pt4) cout<<"Pt1 && Pt4 istinno\n";<br>'
	  +' else cout<<"Pt1 && Pt4 logno\n";     			<br>'          
	+'	 return 0;<br>'
+'} <i>Pt1 ne raven Pt2<br>'
+'Pt1 raven Pt3<br>'
+'Pt1 && Pt3 istinno<br>'
+'Pt1 && Pt4 logno</i><br>'
+'</li></ul>'
...
let per;
per='<table><td id="lev">'
+'<ul><h4>Перегрузка оператора присваивания</h4>'
+'<i>Подробности и горячие факты:</i><br>'
+'<li type="square">операторная функция оператора присваивания не может быть объявлена '
+'как глобальная функция(функция члена класса)</li>'
+'<li type="square">она не наследуется производным классом</li>'
+'<li type="square">компилятор не в состоянии сгенерировать подобную функцию, если '
+'она не является членом класса</li>'
+'<i>Перегрузка оператора присваивания(пример):</i><br>'
+'#include<iostream><br>'
+'using namespace std;<br>'
+'class Coord{<br>'
	+'int x,y;<br>'
...
+'    Coord(){x=0;y=0;}<br>'
	+'void GetCoord(int& _x,int& _y){_x=x;_y=y;}<br>'
+'    Coord& operator=(Coord& rhs);<br>'
	+'bool operator==(Coord rhs);};<br>'
+'Coord&<br>'
+'Coord::operator=(Coord& rhs){<br>'
	+'<i>//проверка на присваивание самому себе</i><br>'
+'	if(this==&rhs)<br>'
	+'   return *this;<br>'
+'	x=rhs.x;<br>'
	+'y=rhs.y;<br>'
+'return *this;}<i>//возвратить левую часть</i>   <br>'
+'bool<br>'
+'Coord::operator==(Coord rhs){<br>'
	+'return(x==rhs.x && y==rhs.y);}<br>'
+'int main(){<br>'
	+'system("color 2f");<br>'
+'	setlocale(LC_ALL,"Rus");<br>'
	+'  Coord Pt1(10,20),Pt2(10,25),Pt3(10,20);<br>'
+'	  Pt2=Pt1;<br>'
	  +' if(Pt1==Pt2)cout<<"Pt1 ravna Pt2\n";<br>'
	+'    else cout<<"Pt1 ne ravna Pt2\n";<br>'
+'	  Pt3=Pt2=Pt1;<br>'
	   +'if(Pt1==Pt2)cout<<"Pt1 ravna Pt3\n";<br>'
	  +'  else cout<<"Pt1 ne ravna Pt3\n";		<br>'          
	+'	 return 0;<br>'
+'} <i>//Pt1 ravna Pt2<br>'
+'//Pt1 ravna Pt3</i><br>'
+'<li><a>Перегрузка унарных операторов</a> <i>опять пример:</i><br>'
+'#include<iostream><br>'
+'using namespace std;<br>'
+'class Coord{<br>'
	+'int x,y;<br>'
...
+'	 int x,y;<br>'
	+' Coord PtA(-10,20);<br>'
+'	 <i>//вызов перегруежнного плюс</i><br>'
	+' PtA=+PtA;<br>'
+'	 PtA.GetCoord(x,y);<br>'
	+' cout&lt;&lt;"PtA.x="&lt;&lt;x&lt;&lt;" PtA.y="&lt;&lt;y&lt;&lt;endl;<br>'
+'	 <i>//вызов перегруженного минус</i><br>'
	+' PtA=-PtA;<br>'
+'	 PtA.GetCoord(x,y);<br>'
	 +'cout&lt;&lt;"PtA.x="&lt;&lt;x&lt;&lt;" PtA.y="&lt;&lt;y&lt;&lt;endl;<br>'		          
	+'	 return 0;<br>'
+'} <i>//PtA.x=-10 PtA.y=20<br>'
+'//PtA.x=10 PtA.y=-20</i><br></li><li>'
+'<a>Перегрузка операторов инкремента и декремента</a><br>'
+'<i>ща, покажем как отличить одну от другой</i><br>'
+'#include<iostream><br>'
+'using namespace std;<br>'
+'class Coord{<br>'
	+'int x,y;<br>'
...
+'    Coord(int _x,int _y){x=_x;y=_y;}<br>'
	+'void GetCoord(int& _x,int& _y){_x=x;_y=y;}<br>'
+'   <i>//префиксная форма инкремента</i><br>'
  +' Coord& operator++();<br>'
+'   <i>//постфиксная форма инкремента</i><br>'
  +' Coord operator++(int); <i>//int указывает на то, что это постфиксная форма</i><br>'
+'   <i>//префиксная форма декремента</i><br>'
  +' Coord& operator--();<br>'
+'   <i>//постфиксная форма декремента</i><br>'
  +' Coord operator--(int);};<br>'
+'Coord&<br>'
+'Coord::operator++(){<br>'
	+'x++;<br>'
+'	y++;<br>'
...
+'	setlocale(LC_ALL,"Rus");<br>'
	+'    int x,y;<br>'
+'	Coord PtA(10,20),PtB(15,25),PtC;<br>'
	+'<i>//префиксная форма инкремента</i><br>'
+'	++PtA;<br>'
	+'PtA.GetCoord(x,y);<br>'
+'	cout&lt;&lt;"(++PtA).x="&lt;&lt;x&lt;&lt;" (++PtA.y=)"&lt;&lt;y&lt;&lt;endl;<br>'
...
+'	--PtA;<br>'
	+'PtA.GetCoord(x,y);<br>'
+'	cout&lt;&lt;"(--PtA).x="&lt;&lt;x&lt;&lt;" (--PtA).y="&lt;&lt;y&lt;&lt;endl;<br>'
	+'<i>//постфиксаня форма декремента</i><br>'
+'	PtA--;<br>'
	+'PtA.GetCoord(x,y);<br>'
+'	cout&lt;&lt;"(PtA--).x="&lt;&lt;x&lt;&lt;" (PtA--).y="&lt;&lt;y&lt;&lt;endl;<br>'
	+'<i>//проверка постфиксной формы декремента</i><br>'
+'	PtC=PtB++;<br>'
	+'PtC.GetCoord(x,y);<br>'
+'	cout&lt;&lt;"PtC.x= "&lt;&lt;x&lt;&lt;" PtC.y="&lt;&lt;y&lt;&lt;endl;<br>'
	+'PtB.GetCoord(x,y);<br>'
+'	cout&lt;&lt;"PtB.x="&lt;&lt;x&lt;&lt;" PtB.y="&lt;&lt;y&lt;&lt;endl;<br>'	 
	+'	 return 0;<br>'
+'} <i>(++PtA).x=11 (++PtA.y=)21<br>'
+'(PtA++).x=12 (PtA++).y=22<br>'
+'(--PtA).x=11 (--PtA).y=21<br>'
+'(PtA--).x=10 (PtA--).y=20<br>'
+'PtC.x= 15 PtC.y=25<br>'
+'PtB.x=16 PtB.y=26</i><br>'
+'</li></ul>'
+'</td><td id="prav">'
+'</td></table>'
